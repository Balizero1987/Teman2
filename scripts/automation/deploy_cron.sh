#!/bin/bash
# Deploy Cron Job for Daily Report (Injection Strategy)
...
echo "🚀 Deploying Daily Reporter Cron to $APP_NAME..."

# 1. Clean up existing machine to allow redeployment (avoid already_exists error)
echo "🧹 Cleaning up existing machine..."
fly machine destroy daily-reporter-cron --app "$APP_NAME" --force || true

# 2. Define the Python Script
CMD=$(cat <<'EOF'
# Check if it's 10:00 AM Bali Time (UTC+8) = 02:00 UTC
# We run this every hour, but only execute logic at 02:xx UTC
current_hour=$(date +%H)
echo "🕒 Current Machine Hour (UTC): $current_hour"

# Logic Gate: Only run if Hour is 02 (10 AM Bali) OR FORCE_RUN is set
if [ "$current_hour" = "02" ] || [ "$FORCE_RUN" = "1" ]; then
    echo "✅ Time Match (10 AM Bali) or Forced. Running Report..."
...
async def main():
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            # --- PATCH: Force Ruslana Offline ---
            # SKIPPING: team_online_status is a view and cannot be updated directly. 
...
PYTHON
    ...
    echo "💤 Not 10 AM Bali (02 UTC) yet. Sleeping..."
fi
EOF
)

# 3. Create the Cron Machine
# Schedule: Hourly (to check logic gate)
fly machine run "$IMAGE_REF" \
  --app "$APP_NAME" \
  --region "$REGION" \
  --name "daily-reporter-cron" \
  --schedule "hourly" \
  --vm-memory "2048" \
...
echo "✅ Hourly check deployed. Will report at 10 AM Bali daily."
