...
class TestSaveConversation:
    ...
    async def test_save_conversation_memory_cache_error(self, conversation_service, mock_db_pool):
        ...
        with patch("services.misc.conversation_service.get_memory_cache", side_effect=Exception("Cache error")):
            result = await conversation_service.save_conversation(
                user_email="test@test.com",
                messages=[{"role": "user", "content": "Hello"}],
            )
    ...
    async def test_save_conversation_triggers_auto_crm(self, conversation_service, mock_db_pool):
        ...
        mock_auto_crm.process_conversation = AsyncMock(return_value={"processed": True, "extracted": {}})
    ...
    @pytest.mark.asyncio
    async def test_save_conversation_empty_messages_no_crm(self, conversation_service, mock_db_pool):
...
class TestGetHistory:
    ...
    async def test_get_history_with_session_id(self, conversation_service, mock_db_pool):
        ...
        mock_row = MagicMock()
        mock_row.__getitem__ = lambda self, key: {"messages": [{"role": "user", "content": "Hello"}]}.get(key, None)
        conn.fetchrow = AsyncMock(return_value=mock_row)
    ...
    async def test_get_history_db_error_fallback_to_cache(self, conversation_service, mock_db_pool):
        ...
        mock_cache = MagicMock()
        mock_cache.get_conversation = MagicMock(return_value=[{"role": "user", "content": "Cached"}])
