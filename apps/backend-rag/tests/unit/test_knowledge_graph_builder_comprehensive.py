...
from backend.services.autonomous_agents.knowledge_graph_builder import (
    KnowledgeGraphBuilder, Entity, Relationship, EntityType, RelationType
)
...
class TestKnowledgeGraphBuilderComprehensive:
    ...
    async def test_extract_entities_facade_llm(self, mock_llm_gateway):
        ...
        mock_llm_gateway.conversational.return_value = {
            "text": json.dumps({
                "entities": [{"id": "e1", "type": "PERMIT", "name": "NIB", "description": "Test"}],
                "relationships": []
            })
        }
    ...
    async def test_export_formats(self):
        ...
        r1 = Relationship(relationship_id="r1", source_entity_id="e1", target_entity_id="e2", relationship_type="LINK")
    ...
    async def test_refresh_from_db(self, mock_db_pool):
        ...
        mock_db_pool.fetch.side_effect = [
            [{"entity_id": "db_node", "entity_type": "TYPE", "name": "Name", "description": "Desc", "properties": "{}", "confidence": 1.0, "source_collection": "coll", "source_chunk_ids": ["c1"]}],
            [{"relationship_id": "db_rel", "source_entity_id": "s", "target_entity_id": "t", "relationship_type": "REL", "properties": "{}", "confidence": 1.0, "source_collection": "coll", "source_chunk_ids": ["c2"]}]
        ]
    ...
    async def test_build_graph_from_collection_hybrid(self, mock_llm_gateway):
        ...
        mock_search.search = AsyncMock(return_value={
            "results": [
                {"id": "chunk_1", "text": "Content about KBLI 56101"}
            ]
        })
    ...
    def test_escape_xml_graphml(self):
        ...
        builder = KnowledgeGraphBuilder()
        builder.entities = {
            "e1": Entity(entity_id="e1", entity_type="TEST", name="Node & Name <tag>", description="Desc")
        }
