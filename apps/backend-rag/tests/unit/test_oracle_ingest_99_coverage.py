...
class TestOracleIngestRouter99Coverage:
    ...
    def test_document_chunk_model_minimal(self):
        ...
        try:
            ...
            chunk = DocumentChunk(content="Minimal valid content", metadata={})
    ...
    def test_ingest_request_model_validation(self):
        ...
        try:
            from app.routers.oracle_ingest import DocumentChunk, IngestRequest

            # Test with custom collection
            chunk = DocumentChunk(content="Test content", metadata={"category": "test"})

            request = IngestRequest(collection="custom_collection", documents=[chunk])
    ...
    async def test_ingest_documents_success(self):
        ...
        try:
            from app.routers.oracle_ingest import DocumentChunk, IngestRequest, ingest_documents

            # Mock search service
            mock_search_service = MagicMock()
            mock_search_service.ingest_documents = AsyncMock(
                return_value={
                    "processed": 5,
                    "failed": 0,
                    "errors": [],
            ...
            with patch(
                "app.routers.oracle_ingest.get_search_service", return_value=mock_search_service
            ):
                # Create test request
                chunk = DocumentChunk(
                    content="Test legal document content",
                    metadata={"law_id": "LAW123", "category": "contract"},
                )
                request = IngestRequest(collection="legal_intelligence", documents=[chunk])
    ...
    async def test_ingest_documents_with_failures(self):
        ...
        try:
            ...
            with patch(
                ...
                chunks = [
                    DocumentChunk(content="Valid content 1", metadata={"category": "test"}),
                    DocumentChunk(content="Valid content 2", metadata={"category": "test"}),
                    DocumentChunk(content="Valid content 3", metadata={"category": "test"}),
                    DocumentChunk(content="Valid content 4", metadata={"category": "test"}),
                    DocumentChunk(content="Valid content 5", metadata={"category": "test"}),
                ]

                request = IngestRequest(collection="legal_intelligence", documents=chunks)
    ...
    async def test_ingest_documents_empty_request(self):
        ...
        try:
            from app.routers.oracle_ingest import IngestRequest, ingest_documents
            ...
            with patch(
                ...
                request = IngestRequest(collection="legal_intelligence", documents=[])
                ...
                for i in range(50):
                    chunks.append(
                        DocumentChunk(
                            ...
                            metadata={"law_id": f"LAW{i + 100}", "category": "contract"},
                        )
                    )
    ...
    def test_model_field_types(self):
        ...
        try:
            ...
            chunk = DocumentChunk(content="Test content", metadata={"test": "value"})
            assert isinstance(chunk.content, str)
            assert isinstance(chunk.metadata, dict)

            # Test IngestRequest fields
            request = IngestRequest(collection="test", documents=[chunk])
