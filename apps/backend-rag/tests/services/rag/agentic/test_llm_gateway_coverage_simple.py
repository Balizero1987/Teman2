"""
Simplified coverage tests for LLMGateway.

This test suite provides comprehensive coverage while avoiding import dependencies
...
mock_modules = {
    ...
    "app.metrics.metrics_collector": MagicMock(),
    "app.utils": MagicMock(),
    "app.utils.tracing": MagicMock(),
    "llm": MagicMock(),
    "llm.genai_client": MagicMock(),
    "services": MagicMock(),
    "services.llm_clients": MagicMock(),
...
mock_modules["llm.genai_client"].GENAI_AVAILABLE = True
mock_modules["services.llm_clients.pricing"].create_token_usage = Mock(
...
class MockCircuitBreaker:
    ...
    def record_failure(self):
        self.failures += 1
        self.last_failure_time = Mock()


mock_modules["app.core.circuit_breaker"].CircuitBreaker = MockCircuitBreaker

# Mock ErrorClassifier
mock_modules["app.core.error_classification"].ErrorClassifier = Mock()
mock_modules["app.core.error_classification"].ErrorClassifier.classify_error = Mock(
    return_value=("quota", "high")
)
mock_modules["app.core.error_classification"].get_error_context = Mock(return_value={})

# Mock tracing functions
mock_modules["app.utils.tracing"].trace_span = Mock()
mock_modules["app.utils.tracing"].set_span_attribute = Mock()
mock_modules["app.utils.tracing"].set_span_status = Mock()

# Mock metrics collector
mock_modules["app.metrics"].metrics_collector = Mock()


# Mock OpenRouter client and ModelTier
class MockModelTier:
    RAG = "rag"


mock_modules["services.llm_clients.openrouter_client"].ModelTier = MockModelTier
mock_modules["services.llm_clients.openrouter_client"].OpenRouterClient = Mock()
...
class TestOpenRouterIntegration:
    ...
    def test_get_openrouter_client_error(self, llm_gateway):
        """Test OpenRouter client initialization error."""
        import httpx

        from services.llm_clients.openrouter_client import OpenRouterClient

        OpenRouterClient.side_effect = httpx.HTTPError("Connection failed")
...
class TestSendMessage:
    ...
    async def test_send_message_with_images(self, llm_gateway, mock_genai_client, sample_images):
    ...
    async def test_send_message_cost_limit_reached(self, llm_gateway, mock_genai_client):
        ...
        with patch(
            "services.rag.agentic.llm_gateway.create_token_usage", return_value=mock_token_usage
        ):
            mock_response = Mock()
...
class TestMetricsAndLogging:
    ...
    async def test_metrics_fallback_recorded(self, llm_gateway, mock_genai_client):
        ...
        metrics_collector.record_llm_fallback.assert_called_with(
