...
from app.modules.knowledge.router import get_search_service, semantic_search
...
def knowledge_service_integration(real_qdrant_client):
    ...
    with patch('app.modules.knowledge.service.QdrantClient') as mock_qdrant_class, \
...
class TestKnowledgeServiceIntegration:
    ...
    async def test_query_router_integration(self, knowledge_service_integration):
        ...
        knowledge_service_integration.router.route = MagicMock(
            return_value=expected_collection
        )
    ...
    async def test_error_handling_integration(self, knowledge_service_integration):
        ...
        try:
            ...
            assert "Qdrant connection error" in error_msg or "Search error" in error_msg or "error" in error_msg.lower()
        finally:
    ...
    async def test_cache_integration(self, knowledge_service_integration):
        ...
        # First call
        result1 = await knowledge_service_integration.search(
            query=query,
            user_level=1,
            limit=5
        )
        ...
        result2 = await knowledge_service_integration.search(
            query=query,
            user_level=1,
            limit=5
        )
